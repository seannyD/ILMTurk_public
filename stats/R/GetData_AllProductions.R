...
getFiles = function(folder,filename){
 ...
 # } else{
    ...
    # get last attempt
    #dx = dx[nrow(dx):1,]
    dx = dx[dx$stage=="Reproduction",]
    dx = dx[!duplicated(dx$meaning),]
...
getResults = function(filename){
  ...
  physicalspace = sapply(dx$phys,function(X){as.numeric(strsplit(X,',')[[1]])})
  hzspace = sapply(dx$Hz,function(X){as.numeric(strsplit(X,',')[[1]])})
  
  return(list(physicalspace=physicalspace,hzspace = hzspace, meanings=dx$meaning))  # physical space
  } else{
    return(list(physicalspace=data.frame(NA,NA,NA),meanings=c(NA,NA,NA)))
  }
}
...
processSignals = function(folder,log,runName =""){
  ...
  for(details in unique(paste(log$logFile,log$chain,log$curvature, sep='_'))){
    ...
    for(i in 1:nrow(lx)){
      ...
      resxx = getResults(physFile)
      sigx = resxx[[1]]
      meaningsxx = resxx[[3]]
      attemptVector = as.vector(unlist(tapply(meaningsxx,meaningsxx,function(X){1:length(X)})))
      sigd = sapply(sigx,sigStats, curvature=curvx2)
      sigx.hzDAT = resxx[[2]] # Hz space
      hzFile = NA#sigx.hzDAT[[2]]
      sigx.hz = sapply(sigx.hzDAT,as.vector)
  ...
  names(signals) = c(
    ...
    'prop.time.plateau.hz','switches.between.sections.hz','steepness.sig.mean.hz','steepness.sig.mean.max.hz','expressiveness.sig.mean.max.hz','logfile','chain','curvature','run','gen','meaning','attempt','physSignal','hzSignal','numLearningRounds','abortedReproductionAttempts','physSigDistinctiveness','hzSigDistinctiveness','dataFilePhysSignal',"dataFileHzSignal")
  ...
  # meanings go from 1 to 3
  signals$meaning= as.numeric((as.character(signals$meaning)))+1
  signals$attempt = as.numeric(signals$attempt)
...
# Weird case of meaning = 4??
signals_MT[signals_MT$meaning==4,]$meaning=1
...
dx = signals_MT
pdf("graphs/signalExamples/SteepnessExamples_AllProductions.pdf",
    height=20)
par(mfrow=c(6,3))
...
write.csv(signals_Nij,"Data/Signals_Nij_AllProductions.csv",row.names=F)
write.csv(signals_MT,"Data/Signals_MT_AllProductions.csv",row.names=F)
write.csv(signals_SONA,"Data/Signals_SONA_AllProductions.csv",row.names=F)
...
Outliers are identified using the *influence.ME* package.

```{r outliersSONA, cache=T}
finalModel=  lmer(prop.time.plateau.norm~
        1 + 
          curvature.center +
          gen +
...
inf.finalModel = influence(finalModel, obs = T)
cooksd = cooks.distance(inf.finalModel)
plot(cooksd)
outlier = which(cooksd==max(cooksd))
```

There is clearly one outlier that is influencing the model more than others (`r paste(dx[outlier,]$chain,', generation',dx[outlier,]$gen+5,', meaning', dx[outlier,]$meaning,', curvature =',dx[outlier,]$curvature)`; the signal is entierly within the steep region).  

The same datapoint is also an outlier in terms of steepness:

```{r outliersSONASteepness, cache=T}
finalModel.steepness=lmer(steepness.sig.mean.max.norm~
        1 + 
          curvature.center +
          gen +
          curvature.center : gen +
...
inf.finalModel.steepness = influence(finalModel.steepness, obs = T)
cooksd = cooks.distance(inf.finalModel.steepness)
plot(cooksd)
```
...
(Curvature and generation do not vary within participants, so are not suitable for random slopes by participant).

From the tests above, we determine that no random slopes are required.  
...
There is one outlying signal - a flat tone in the steep region ((`r paste(dx[outlier,]$chain,', generation',dx[outlier,]$gen+5,', meaning', dx[outlier,]$meaning,', curvature =',dx[outlier,]$curvature)`).  We remove this outlier
...
```{r}
controlX = lmerControl(optimizer = 'bobyqa')
...
# What predicts signal steepness?

Use a random forest analysis to see whether any other variables predict the steepness of the signal.

```{r randomForest, cache=T}
ct = cforest(steepness.sig.mean.max~
               length + averageSlope + 
               propZero + switches + 
               maxslope + slope.move + 
               jerkyness + switches.between.sections + 
               length.hz + 
               averageSlope.hz + propZero.hz + 
               switches.hz + maxslope.hz + 
               slope.move.hz + jerkyness.hz + 
               meaning + numLearningRounds + 
...
This suggests that the number of switches affects the steepness measure.  This measure essentially splits into two categories: 0-1 and 2+.  Interestingly, this means that if there are no or few switches between sections, participants tend to use the steep sections (which makes sense).

```{r warning=F, message=F}
plotmeans(steepness.sig.mean.max ~ switches.between.sections,
          data = dx)
```
...
```{r}
dx$switches.between.sections.2 = dx$switches.between.sections >1
```
...
m0.5= lmer(steepness.sig.mean.max.norm~
        1 + 
        switches.between.sections.2 +
        (1 | chain2) + 
        (1 | participant) +
        (1 | meaning), 
        data = dx)
...
m4= lmer(steepness.sig.mean.max.norm~
        1 + 
          ...
          gen +
          curvature.center : gen +
          curvature.center :  switches.between.sections.2 +
        (1 | chain2) + 
        (1 | participant) +
        (1 | meaning), 
        data = dx)

anova(m0,m0.5,m1,m2,m3)
```

Now there's a main effect of generation?

# What predicts generation?


```{r generationCT, cache=T}
ct = cforest(gen ~
               steepness.sig.mean.max + 
               length + averageSlope + 
               propZero + switches + 
               maxslope + slope.move + 
               jerkyness + switches.between.sections + 
